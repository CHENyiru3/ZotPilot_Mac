...
> **维护规则**：每次新增、修改或退休 MCP 工具时，必须同步检查**本仓库根目录的 `SKILL.md`**：
> 1. **MCP Tool Reference 表格** — 增删工具条目，确保与实现一致
> 2. **运行逻辑流程**（Research / Error Recovery 等节）— 检查工具调用顺序、参数、返回字段是否与实现对齐；如有行为变更（如 preflight 策略、路由优先级、verbosity 默认值）必须同步更新描述
>
> 注意：本机 agent 安装目录中的 `SKILL.md` **不直接修改**，用户通过升级 ZotPilot 包（`pip install -U zotpilot` 或重新 clone）获取最新版本。
...
ZotPilot 是一个 **FastMCP 服务器**，提供 **18 个原子工具** 用于对本地 Zotero 文献库进行语义搜索和入库整理。架构分四层：

### 1. 入口点
- `cli.py` — argparse CLI，子命令：`setup`、`index`、`status`、`doctor`、`config`、`register`、`upgrade`（`update` 别名）。无子命令时启动 MCP 服务器。
- `server.py` — 薄层 shim：import `state.mcp` 和 `tools/` 包（import 副作用注册所有工具），启动时检查 skill drift，然后调用 `mcp.run()`。

### 2. MCP 状态与懒加载单例（`state.py`）
所有共享对象（`VectorStore`、`Retriever`、`Reranker`、`ZoteroClient`、`ZoteroWriter`、`ZoteroApiReader`、`IdentifierResolver`）均为懒加载单例，由单一 `threading.Lock` 保护。工具在每次请求时调用 `_get_retriever()`、`_get_zotero()` 等。后台线程监控父进程 PID，父进程退出时调用 `os._exit(0)` 防止孤儿服务器进程。

### 3. 工具模块（`tools/`）
18 个原子工具分布在 8 个模块，均由 `tools/__init__.py` import 触发 `@mcp.tool` 装饰器注册：

| 模块 | 工具 |
|------|------|
| `search.py` | `search_papers`（含 `section_type` tables/figures）、`search_topic`、`search_boolean`、`advanced_search` |
| `context.py` | `get_passage_context` |
| `library.py` | `get_paper_details`、`browse_library`、`get_notes`、`get_annotations`、`profile_library` |
| `indexing.py` | `index_library`（含 `item_keys` 局部重索引）、`get_index_stats` |
| `citations.py` | `get_citations`（方向参数 refs/citing/count） |
| `write_ops.py` | `create_note`、`manage_tags`、`manage_collections`（含 `action="create"`） |
| `admin.py` | `switch_library`（代码保留但不注册为 MCP 工具，v0.6.0+ 恢复） |
| `ingestion/` | `search_academic_databases`、`ingest_by_identifiers` |

**入库核心**：`ingest_by_identifiers` 是**同步原子工具**，内部完成 Connector preflight → 逐条 save → **本地 API 验证**（`validate_saved_item`）→ 失败时 DOI API fallback → PDF 验证。不再有 batch 状态机，不再有 approve/confirm 等 phase gate 工具。当返回 `action_required` 非空时（anti-bot 或 connector_offline），Agent 停下等用户处理。

写操作（`write_ops.py`）需要 `ZOTERO_API_KEY` + `ZOTERO_USER_ID` 环境变量，使用 `ZoteroWriter`（pyzotero Web API）。只读工具使用 `ZoteroClient`（本地 SQLite）。验证/删除使用 **Zotero Desktop 本地 HTTP API（port 23119）** 避免 Web API 同步延迟。
...
- **双重检查锁定的单例模式**：所有昂贵对象每个服务器进程初始化一次，MCP server 重启时重置。
...
`connector/` 目录包含面向 agent researcher 的 Zotero 浏览器扩展，基于官方 [zotero-connectors](https://github.com/zotero/zotero-connectors) fork。

**定位**：在官方 connector 基础上增加 WebSocket/API 层，让 AI agent 能通过 `save_from_url` / `save_urls` 工具直接触发带机构权限的 PDF 抓取。

**技术栈**：JavaScript、gulp 构建、支持 Chrome (MV3) / Firefox / Safari
...
# 详见 connector/README.md
```

### 版本管理

connector 与 MCP server 共用版本号（`pyproject.toml` 为 source of truth）。发版时两个组件同步发布，使用同一个 git tag（`vX.Y.Z`）。

connector 的 `package.json` 版本需与 `pyproject.toml` 保持一致，发版清单中需检查。

### 发版清单补充

在原有发版清单基础上额外检查：
- [ ] `connector/package.json` 版本与 `pyproject.toml` 一致
- [ ] `connector/CHANGELOG.md` 有对应版本条目（如有 connector 变更）
