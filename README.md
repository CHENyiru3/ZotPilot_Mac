...
- `zotpilot register` / `zotpilot install` 只把 ZotPilot MCP 接到已检测的 AI 客户端，并刷新 packaged skills
...
MCP 工具没出现？重新注册并重启 agent：`zotpilot install`。
...
| 症状 | 怎么办 |
|------|------|
| 找不到 Skill | `zotpilot install` 然后重启 agent |
| `zotpilot: command not found` | 先 `pip install zotpilot` |
| MCP 工具没出来 | `zotpilot install` 然后重启 agent |
