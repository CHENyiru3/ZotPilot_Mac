...
| `ztp-setup` | 指导 agent 调用 `zotpilot setup` / `register` / `doctor` 做安装、注册、升级、排错。它不是 CLI 命令本身 |
...
- `zotpilot setup` 写共享本地配置到 `~/.config/zotpilot/config.json`
- `zotpilot config set` 管理共享配置和安全凭证
- `zotpilot register` 只把 ZotPilot MCP 接到已检测的 AI 客户端
- secrets 不写入 `config.json`，也不写入客户端配置

敏感凭证默认存到 ZotPilot 自己管理的 secure store（macOS Keychain；不可用时可显式启用本地受限文件）。环境变量仍可作为临时 override：
...
MCP 工具没出现？重新注册并重启 agent：`zotpilot register`。
...
升级当前 ZotPilot CLI、刷新 Skill 文件、同步 MCP runtime 配置。
...
| `upgrade`（不带参数） | 升级 CLI + 刷新 skills + 同步 runtime |

> 开发模式（`zotpilot register --dev`）：`upgrade` 会提示代码更新请 `git pull`，但仍执行 runtime 同步。
...
| 症状 | 怎么办 |
|------|------|
| 找不到 Skill | `zotpilot register` 然后重启 agent |
...
更多见 [troubleshooting.md](src/zotpilot/references/troubleshooting.md)。
