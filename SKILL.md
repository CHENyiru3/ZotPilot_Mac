...
| 3b | `doi` only (or URL is pdf/doi.org link) | `ingest_papers([{doi:...}])` — DOI auto-resolved to publisher page; saved via Connector when available (with PDF), falls back to CrossRef API if Connector unavailable. |
